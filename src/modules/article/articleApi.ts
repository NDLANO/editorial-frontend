...
export const searchArticles = (locale: string, queryString = ''): Promise<ArticleSearchResult> =>
  fetchAuthorized(`${articleUrl}/?language=${locale}&fallback=true${queryString}`).then(
    resolveJsonOrRejectWithError,
  );

export const searchRelatedArticles = async (input: string, locale: string, contentType: string) => {
...
export const getArticleFromArticleConverter = (id: number, locale: string) =>
  fetchAuthorized(`${articleConverterUrl}/json/${locale}/${id}`).then(resolveJsonOrRejectWithError);
