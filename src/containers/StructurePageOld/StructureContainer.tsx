...
export const StructureContainer = () => {
  ...
  const [subject, topic, ...rest] = location.pathname.replace('/structureOld/', '').split('/');
  ...
  const handleStructureToggle = (input: { path: string }) => {
    ...
    const currentPath = location.pathname.replace('/structureOld/', '');
    ...
    navigate(`/structureOld/${newPath.concat(deleteSearch ? '' : search)}`);
  };
