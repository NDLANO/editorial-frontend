...
describe('Selecting text and using the toolbar', () => {
  ...
  it('change the text styling', () => {
    ...
    cy.get('[data-slate-node=element] > p').should('be.visible').first().click();
  ...
  it('can create a valid link', () => {
    cy.get('[data-cy=slate-editor] [data-slate-editor=true]')
      ...
      .focus()
      .then(($el) => {
        cy.wrap($el).click().type('This is a test link{leftArrow}{leftArrow}').blur();
  ...
  it('All lists work properly', () => {
    cy.get('[data-cy=slate-editor] [data-slate-editor=true]')
      ...
      .focus()
      .then(($el) => {
        cy.wrap($el).click().type('First item in list');
        cy.wrap($el).type('{selectAll}');
  ...
  it('Definition list work properly', () => {
    cy.get('[data-cy=slate-editor] [data-slate-editor=true]')
      ...
      .focus()
      .then(($el) => {
        cy.wrap($el).click().type('Definition Term in list');
        cy.wrap($el).type('{selectAll}');
        ...
        cy.wrap($el).type('{end}{enter}').tab().type(' Definition Description in list');
  ...
  it('Selecting multiple paragraphs gives multiple terms', () => {
    cy.get('[data-cy=slate-editor] [data-slate-editor=true]')
      ...
      .focus()
      .then(($el) => {
        cy.wrap($el).click().type('Definition Term 1');
