...
const ALT_TEXT_QUERY: AlttextQuery = {
  ...
  nn: `<Instructions>
            ...
            Du har fått som oppgåve å skrive en alternativ tekst for bilete vedlagt på NO. Beskrivinga skal vere på maks 125 teikn.
...
const REFLECTION_QUESTION_QUERY: ReflectionQuery = {
  en: (text: string) => `<draft>${text}</draft>
          ...
          Your task is to read through the article in <draft> og write five reflection questions based on the article in NB. The questions must be on a level which fits students in high school.
