...
export const search = (query, page, locale) =>
  fetchWithAccessToken(`${baseUrl}/?${queryString.stringify({ query, page })}&page-size=16&language=${locale}`).then(resolveJsonOrRejectWithError);
...
export const fetchImage = imageId =>
  fetchWithAccessToken(`${baseUrl}/${imageId}`).then(resolveJsonOrRejectWithError);
