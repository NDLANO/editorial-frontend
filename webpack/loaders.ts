...
export const loaders = (
  ...
  const agnosticCssLoaders: RuleSetUseItem[] = [
    {
      ...
      options: {
        ...
        modules: {
          auto: true,
          localIdentName:
            env !== 'production' ? '[path][name]__[local]' : '[name]__[local]__[hash:base64:5]',
  ...
  const clientCssLoaders: RuleSetUseItem[] = [
    env === 'development' ? { loader: 'style-loader' } : { loader: MiniCssExtractPlugin.loader },
  ];
  ...
  return [
    ...
    {
      test: /\.css$/,
      use: type === 'client' ? clientCssLoaders.concat(agnosticCssLoaders) : agnosticCssLoaders,
    },
  ];
};
